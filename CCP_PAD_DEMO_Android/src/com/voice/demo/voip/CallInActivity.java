		...
		try {
			if (isConnect) {
				...
				if (mCallType == Device.CallType.VIDEO) {
					...
					/*
					 * if(mVideoStop.isEnabled()) { logText(getString (R.string.str_video_call_end, CCPConfig.VoIP_ID.substring(CCPConfig.VoIP_ID.length() - 3, CCPConfig.VoIP_ID.length()))); } else {
					 */
