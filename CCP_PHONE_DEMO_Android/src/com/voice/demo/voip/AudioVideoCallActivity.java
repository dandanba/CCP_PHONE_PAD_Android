 ...
 * 
 * <p>
 * Title: AudioVideoCallActivity.java
 * </p>
 * <p>
 * Description:
 * </p>
 ...
 * @version 3.5
 */
public class AudioVideoCallActivity extends BaseChatActivity implements OnVoIPListener.OnCallRecordListener {
	...
	public void initProwerManager() {
		mWakeLock = ((PowerManager) getSystemService("power")).newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP, "CALL_ACTIVITY#" + super.getClass().getName());
	...
	protected void onResume() {
		...
		callRecordEnabled = CcpPreferences.getSharedPreferences().getBoolean(CCPPreferenceSettings.SETTING_CALL_RECORDING.getId(), (Boolean) CCPPreferenceSettings.SETTING_CALL_RECORDING.getDefaultValue());
	...
	protected void onStop() {
		...
		if (isConnect) {
			if (isIncomingCall) {
				CCPNotificationManager.showInCallingNotication(getApplicationContext(), mCallType, getString(R.string.voip_is_talking_tip), null);
			} else {
				CCPNotificationManager.showOutCallingNotication(getApplicationContext(), mCallType, getString(R.string.voip_is_talking_tip), null);
			}
		}
	}

	public void DisplayLocalSurfaceView() {
		if (mCallType == Device.CallType.VIDEO && mLoaclVideoView != null && mLoaclVideoView.getVisibility() == View.VISIBLE) {
	...
	protected void onReceiveBroadcast(Intent intent) {
		 ...
		 * 
		 * // If disconnection with cloud communication server port, then hang up the local call doHandUpReleaseCall(); CCPApplication.getInstance().showToast(R.string.ccp_http_err_voip); }
		 */
	}
	 ...
	 * 
	 * @param reason
	 *            recording status: 0: Success -1: Recording failed then delete the recording file -2: Recording failed of write file but still to retain the record file.
	 */
	@Override
